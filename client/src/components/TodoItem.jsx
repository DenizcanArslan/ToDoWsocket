"use client";

import { useDispatch } from 'react-redux';
import { updateTodoStatus, removeTodo } from '@/store/todoSlice';

export default function TodoItem({ todo }) {
  const { id, text, completed, createdAt, updatedAt } = todo;
  const dispatch = useDispatch();

  const formattedDate = new Date(createdAt).toLocaleDateString('tr-TR', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    hour: '2-digit',
    minute: '2-digit'
  });

  const handleToggle = () => {
    dispatch(updateTodoStatus({ id, completed: !completed }));
  };

  const handleDelete = () => {
    dispatch(removeTodo(id));
  };

  return (
    <div className={`p-4 border rounded-lg ${completed ? 'bg-gray-100' : 'bg-white'}`}>
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            checked={completed}
            onChange={handleToggle}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-blue-500 focus:ring-blue-500 cursor-pointer"
          />
          <div>
            <p className={`${completed ? 'line-through text-gray-500' : 'text-gray-900'}`}>
              {text}
            </p>
            <p className="text-xs text-gray-500 mt-1">{formattedDate}</p>
          </div>
        </div>
        <button
          onClick={handleDelete}
          className="text-red-500 hover:text-red-700 transition-colors cursor-pointer"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
          </svg>
        </button>
      </div>
    </div>
  );
} 